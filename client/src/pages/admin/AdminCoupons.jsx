...
export default function AdminCoupons() {
  ...
  const handleSave = async () => {
    ...
    if (form.appliesTo === 'selected' && form.applicableProductIds.length === 0) return toast.error('Select at least one product');
    setSaving(true);
    try {
      const payload = {
        code: form.code,
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Coupons ({coupons.length})</h1>
          <p className="text-sm text-gray-500 mt-1">Create and manage discount codes customers can apply at checkout</p>
        </div>
      ...
      {coupons.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center gold-border">
          ...
          <p className="text-gray-500 text-lg font-medium">No coupons yet</p>
          <p className="text-gray-400 text-sm mt-1">Create your first promo code to drive sales</p>
          <button onClick={openAdd} className="btn-gold rounded-xl mt-4">Add Coupon</button>
        </div>
      ) : (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <span className="flex items-center gap-1 text-2xl font-bold text-gray-800">
                      ...
                      {coupon.discountType === 'percent' ? `${coupon.value}% OFF` : `₹${coupon.value.toLocaleString()} OFF`}
                    </span>
                  </div>
                  <div className="space-y-1 text-xs text-gray-500">
                    <p>Min. order: <span className="font-medium text-gray-700">₹{coupon.minOrder.toLocaleString()}</span></p>
                    <p className="flex items-center gap-1"><Package size={12} className="text-gold-500" /> Applies to: <span className="font-medium text-gray-700">{coupon.appliesTo === 'selected' ? `${(coupon.applicableProductIds || []).length} selected product${(coupon.applicableProductIds || []).length === 1 ? '' : 's'}` : 'All products'}</span></p>
                    {coupon.maxDiscount ? <p>Max discount: <span className="font-medium text-gray-700">₹{coupon.maxDiscount.toLocaleString()}</span></p> : <p>Max discount: <span className="font-medium text-gray-700">No limit</span></p>}
                    <p>Uses: <span className="font-medium text-gray-700">{coupon.usedCount || 0}{coupon.maxUses > 0 ? ` / ${coupon.maxUses}` : ' / ∞'}</span></p>
                    <p>Valid: <span className="font-medium text-gray-700">{coupon.validFrom ? new Date(coupon.validFrom).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : 'Any time'} — {coupon.validTo ? new Date(coupon.validTo).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'No expiry'}</span></p>
                  </div>
                </div>
                <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-100">
                  <button onClick={() => handleEdit(coupon)} className="flex-1 p-2 text-gold-600 hover:bg-gold-50 rounded-lg transition text-sm font-medium">Edit</button>
                  <button onClick={() => handleDelete(coupon._id)} className="flex-1 p-2 text-red-500 hover:bg-red-50 rounded-lg transition text-sm font-medium">Delete</button>
                </div>
              </div>
            );
          })}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-gold-200 bg-gradient-to-r from-gold-50 to-white sticky top-0 z-10">
              <h2 className="text-lg font-bold gold-text">{editingId ? 'Edit Coupon' : 'Add New Coupon'}</h2>
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex gap-2">
                  {['percent', 'fixed'].map(t => (
                    <button key={t} onClick={() => setForm({ ...form, discountType: t })}
                      className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium border-2 transition ${form.discountType === t ? 'border-gold-500 bg-gold-100 text-gold-700' : 'border-gray-200 text-gray-500 hover:border-gold-300'}`}>
              ...
              {form.appliesTo === 'selected' && (
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="block text-sm font-medium text-gray-700">Select Products <span className="text-gold-600 font-bold">({form.applicableProductIds.length} selected)</span></label>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{form.discountType === 'percent' ? 'Discount % *' : 'Discount Amount (₹) *'}</label>
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-gold-200 bg-gold-50/30 sticky bottom-0">
              ...
              <button onClick={handleSave} disabled={saving} className="btn-gold rounded-xl flex items-center gap-2">
                <Save size={16} /> {editingId ? 'Update Coupon' : 'Create Coupon'}
              </button>
            </div>
          </div>
        </div>
