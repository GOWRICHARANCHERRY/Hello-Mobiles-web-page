  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Enquiries & Leads ({leads.length})</h1>
          <p className="text-sm text-gray-500 mt-1">{unreadCount} unread — newsletter signups and contact enquiries</p>
        </div>
      ...
      {leads.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center gold-border">
          ...
          <p className="text-gray-400 text-sm mt-1">Newsletter signups and contact enquiries will appear here</p>
        </div>
      ) : (
            ...
            <div key={lead._id} className={`bg-white rounded-2xl shadow-sm gold-border p-4 flex flex-col md:flex-row md:items-center gap-4 transition ${lead.read ? 'opacity-70' : 'ring-2 ring-gold-300'}`}>
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1.5">
                  {!lead.read && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-gold-100 text-gold-700 uppercase">New</span>}
                ...
                <p className="font-semibold text-gray-800 flex items-center gap-2 truncate">
                  {lead.name ? <><User size={14} className="text-gold-500 flex-shrink-0" />{lead.name}</> : 'Anonymous visitor'}
                </p>
              ...
              <div className="flex items-center gap-2 md:flex-col md:items-end flex-shrink-0">
                {lead.read ? (
                  <span className="flex items-center gap-1.5 text-xs text-green-600 font-medium"><Check size={14} /> Read</span>
                ) : (
