...
export default function AdminDashboard() {
  ...
  return (
    <div className="animate-fade-in">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">{t('admin.dashboardTitle')}</h1>

      {/* Stats Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          { label: t('admin.todaysRevenue'), value: `₹${(data?.dailyRevenue || 0).toLocaleString()}`, icon: DollarSign, color: 'bg-green-100 text-green-600' },
          { label: t('admin.monthlyRevenue'), value: `₹${(data?.monthlyRevenue || 0).toLocaleString()}`, icon: TrendingUp, color: 'bg-blue-100 text-blue-600' },
          { label: t('admin.totalOrders'), value: data?.totalOrders || 0, icon: ShoppingCart, color: 'bg-purple-100 text-purple-600' },
          { label: t('admin.pendingOrders'), value: data?.pendingOrders || 0, icon: Package, color: 'bg-yellow-100 text-yellow-600' },
        ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-lg font-bold text-gray-800 mb-4">{t('admin.bestSellingProducts')}</h2>
          ...
          ) : <p className="text-gray-400 text-sm text-center py-8">{t('admin.noDataYet')}</p>}
        </div>

        {/* Sales by Brand */}
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-lg font-bold text-gray-800 mb-4">{t('admin.salesByBrand')}</h2>
        ...
        {/* Monthly Trend */}
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-lg font-bold text-gray-800 mb-4">{t('admin.monthlyRevenueTrend')}</h2>
          {data?.monthlyTrend?.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={data.monthlyTrend}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="_id.month" tick={{ fontSize: 11 }} tickFormatter={m => t('admin.monthAbbr').split(',')[m-1]} />
                <YAxis />
                <Tooltip formatter={(v) => `₹${v.toLocaleString()}`} />
        ...
        {/* Sales by Category */}
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-lg font-bold text-gray-800 mb-4">{t('admin.salesByCategory')}</h2>
