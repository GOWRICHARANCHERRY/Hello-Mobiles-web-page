...
import { Home, Search, ShoppingCart, User, Heart, Menu, X, LogOut, Calculator, RotateCcw, LogIn, Mail, Instagram, CreditCard, Package, MapPin, Ticket, HelpCircle, Globe, ChevronDown, Settings, Scale, XCircle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gold-50">
      ...
      {compare.length > 0 && (
        <div className="fixed bottom-6 left-6 z-50 no-print">
          <Link to="/compare" className="flex items-center gap-2 bg-gold-700 text-white pl-4 pr-2 py-2 rounded-full shadow-2xl hover:bg-gold-800 transition-all group">
            <Scale size={16} className="text-gold-200" />
            <span className="text-sm font-semibold">{t('comp.compareCount', { count: compare.length })}</span>
            <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); clearCompare(); }} aria-label={t('cust.compareClear')}
              className="ml-1 w-6 h-6 flex items-center justify-center rounded-full bg-white/15 hover:bg-red-500 transition">
              <XCircle size={14} />
            </button>
          </Link>
        </div>
